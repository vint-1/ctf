...
int main (int argc, char **argv)
{
    ...
    unsigned long timestamp = 1604889890;
    t = (timestamp / 60 - 0x806880) * 2; // (t & -4) for 60 sec periods, (t & -8) for 120 sec periods, etc.
    printf("timestamp = %llu\n", timestamp);

    // Set key
    unsigned char keyb[] = {0xde,0xed,0xa1,0x13,0x7a,0xb0,0x12,0x02};
    for (int xx = 0; xx < 8; xx++) {
        key.B[xx] = keyb[xx];
    }
    // key.Q[0] = 16063672550935237122ULL;

    printf("key = %llu\n", key.Q[0]);
    securid_hash_time(t, &hj, key);

    printf ("%X : %02X%02X%02X\n", i + j, hj.B[0], hj.B[1], hj.B[2]);
    printf ("%X : %02X%02X%02X\n", i + j, hj.B[3], hj.B[4], hj.B[5]);

    for (i = (t & -8), j = (t & -8) - 4; i < (t & -8) + 0x40560; i += 4, j -= 4)
    {
        // key from tokenfile
        // i and j are just to calculate time dift
        // hi/hj is probably result?
